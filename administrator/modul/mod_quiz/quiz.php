    ...
    if ($_SESSION[leveluser]=='admin'){
        ...
        while ($t=mysql_fetch_array($siswa_yangmengerjakan2)){
            ...
            echo "<tr><td>$no</td>                      
                      ...
                      <td>$k[nama]</td>";
                      if ($t[dikoreksi]=='B'){
                          echo "<td>Jawaban soal essay <b>belum dikoreksi</b>
                          ...
                          echo "<td>Jawaban soal essay <b>sudah dikoreksi</b><br>Nilai Tugas/Quiz Pilihan Ganda : $n2[persentase]<br>
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        ...
        while ($t=mysql_fetch_array($siswa_yangmengerjakan2)){
            ...
            echo "<tr><td>$no</td>
                      ...
                      <td>$k[nama]</td>";
                      if ($t[dikoreksi]=='B'){
                          echo "<td>Jawaban soal essay <b>belum dikoreksi</b></td>";
                          echo "
                          <td><a href=$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]>Hapus Siswa</a> |
                          <a href=?module=quiz&act=koreksi&id=$t[id_tq]&id_siswa=$s[id_siswa]>Koreksi Jawaban Esay</a></td></tr>";
                      }else{
                          echo "<td>Jawaban soal essay <b>sudah dikoreksi</b><br>Nilai Tugas/Quiz Essay: $n[nilai]</td>";
                          echo "
                          <td><a href=$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]>Hapus Siswa</a> |
                          <a href=?module=quiz&act=editkoreksi&id=$t[id_tq]&id_siswa=$s[id_siswa]>Edit Koreksi</a></td></tr>";
                      }
...
case "koreksi":
    if ($_SESSION[leveluser]=='admin'){
        $cek = mysql_query("SELECT * FROM siswa_sudah_mengerjakan WHERE id_siswa='$_GET[id_siswa]' AND id_tq='$_GET[id]'");
    ...
    elseif ($c[dikoreksi]=='S'){
         echo "<script>window.alert('Sudah dikoreksi');
         window.location=(href='?module=quiz')</script>";
    }
    }
    elseif ($_SESSION[leveluser]=='pengajar'){
        ...
        $soal_esay = mysql_query("SELECT * FROM quiz_esay WHERE id_tq='$_GET[id]'");
        $esay = mysql_num_rows($soal_esay);
        //echo "SELECT * FROM siswa_sudah_mengerjakan WHERE id_siswa='$_GET[id_siswa]' AND id_tq='$_GET[id]' pilganda = $pilganda, esay = $esay <br/>";
        if (!empty($pilganda) AND !empty($esay)){
            $siswa = mysql_query("SELECT * FROM siswa WHERE id_siswa='$_GET[id_siswa]'");
            $s = mysql_fetch_array($siswa);
            $jawaban = mysql_query("SELECT * FROM jawaban WHERE id_tq='$_GET[id]' AND id_siswa='$_GET[id_siswa]'");
            $cek=mysql_num_rows($jawaban);
            //echo "SELECT * FROM jawaban WHERE id_tq='$_GET[id]' AND id_siswa='$_GET[id_siswa]' cek = $cek, esay = $esay <br/>";
            if (!empty($cek)){
                echo "<h2>Jawaban Soal Essay Siswa <b>$s[nama_lengkap]</b></h2><hr>";
                $no=1;
                while ($j=mysql_fetch_array($jawaban)){
        ...
        $cek_esay = mysql_num_rows($esay);
                ...
                elseif (empty($cek_pilganda) AND empty($cek_esay)){
                    echo"<br><b class='judul'>Nilai Anda</b><br><p class='garisbawah'></p>
                      ...
                      $n_pilganda = mysql_fetch_array($pilganda);
                      $n_esay = mysql_fetch_array($esay);
                      echo "<tr><td>Tugas Pilihan Ganda</td><td>Anda Belum mengerjakan  $_GET[id_topik] $_SESSION[idsiswa]</td></tr>
...
case "infokerjakan":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        $cekqe = mysql_query("SELECT * FROM quiz_esay WHERE id_tq='$_GET[id]'");
        $dataqe = mysql_num_rows($cekqe);
        if ($dataqe>0){
            $esay = mysql_query("SELECT * FROM jawaban WHERE id_tq = '$_GET[id]' AND id_siswa = '$_SESSION[idsiswa]'");
            $cek_esay = mysql_num_rows($esay);
        }else{
            $cek_esay = 1;                
        }            
        //echo "SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq='$_GET[id]' AND id_siswa = '$_SESSION[idsiswa]', dataqe = $dataqe, cek_esay = $cek_esay <br/>";
        //echo "SELECT * FROM jawaban WHERE id_tq = '$_GET[id]' AND id_siswa = '$_SESSION[idsiswa]', dataqe = $dataqe, cek_esay = $cek_esay <br/>";
        if ($data[hits]<=0 OR $cek_esay < 1){
        ...
        echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>
            ...
            3. Pilih browser yang suport dengan Elearning SMP MTSS PUI MAJA yaitu Mozilla Firefox.<br>
